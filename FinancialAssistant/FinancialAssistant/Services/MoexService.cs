...
namespace FinancialAssistant.Services
{
    public class MoexService
    {
        ...
        public static MoexService getInstance()
        {
            if (instance == null)
            {
                instance = new MoexService();
            }
            return instance;
        }
        ...
        public void MoexTimerTools()
        {
            MoexLoadToolsTimer = new Timer(LoadTools, null, TimeSpan.Zero, TimeSpan.FromDays(1));
        }
        public void MoexTimerMarketsBoardsFiles()
        {
            MoexLoadMarketsBoardsFiles = new Timer(LoadMarketsBoardsFiles, null, TimeSpan.Zero, TimeSpan.FromDays(30));
        }
            ...
            foreach (string file in files)
            {
                ...
                if (jobj != null)
                {
                    foreach (JToken token in jobj["history"]!["data"]!)
                    {
                        if (token != null)
                        {
                            using (FinancialAssistantContext db = new FinancialAssistantContext())
                            {
                                Tool dbTool = new Tool();
                                dbTool.ToolName = token[columns["SHORTNAME"]].Value<string?>() ?? "none";
                                dbTool.SecId = token[columns["SECID"]].Value<string?>() ?? "none";
                                dbTool.Market = db.Markets.Where(m => m.MarketName == market).FirstOrDefault();
                                dbTool.Board = db.Boards.Where(b => b.BoardName.Substring(0, 4) == token[columns["BOARDID"]].Value<string?>()).FirstOrDefault();
                                ...
                                TradeDate dbtradeDate = new TradeDate();
                                dbtradeDate.Tool = db.Tools.Where(t => t.ToolName == dbTool.ToolName & t.SecId == dbTool.SecId & t.Board == dbTool.Board & t.Market == dbTool.Market).FirstOrDefault();
                                dbtradeDate.Date = token[columns["TRADEDATE"]].Value<string?>() ?? "none";
                                dbtradeDate.Price = token[columns["CLOSE"]].Value<double?>() ?? 0;
        ...
        public void LoadMarkets()
        {
            ...
            {
                using (HttpResponseMessage responce = client.GetAsync("https://iss.moex.com/iss/engines/stock/markets.json").Result)
                {
                                ...
                                foreach (JToken token in jObj["markets"]["data"])
                                {
                                    if (token != null)
                                    {
                                        using (FinancialAssistantContext db = new FinancialAssistantContext())
                                        {
                                            ...
                                            market.MarketName = token[1].Value<string>();
        ...
        public void LoadBoards(string market)
        {
            using (HttpClient client = new HttpClient())
            {
                using (HttpResponseMessage responce = client.GetAsync($"https://iss.moex.com/iss/engines/stock/markets/{market}/boards.json").Result)
                {
                    if (responce.IsSuccessStatusCode)
                    {
                        string result = responce.Content.ReadAsStringAsync().Result;
                                ...
                                foreach (JToken token in jObj["boards"]["data"])
                                {
                                    if (token != null)
                                    {
                                        using (FinancialAssistantContext db = new FinancialAssistantContext())
                                        {
                                            ...
                                            board.BoardName = token[2].Value<string>() + "|" + token[3].Value<string>();
                                            board.Market = db.Markets.Where(m => m.MarketName == market).FirstOrDefault();
        ...
        public void LoadSamples()
        {
            ...
            foreach (var sample in samples)
            {
                using (HttpClient client = new HttpClient())
                {
                    using (HttpResponseMessage response = client.GetAsync(sample.Value.ToString()).Result)
                    {
        ...
        private string CreateUrl(string market, string date)
        {
            string result = String.Empty;
            result = $"https://iss.moex.com/iss/history/engines/stock/markets/{market}/securities.json?date={date}";
            return result;
        }
        ...
        private bool DayOff(DateTime date)
        {
